...
public class CrackDigest
{
    ...
    public String digest(byte[] dataIn)
    {
        int[] H = {0x67452301, 0xEFCDAB89, 0x98BADCFE, 0x10325476, 0xC3D2E1F0};

        return digestWithIV(dataIn, H, 0);
    }

    public String digestWithIV(byte[] dataIn, int[] H, int lengthPad)
    {
        byte[] paddedData = padTheMessage(dataIn, lengthPad);
    ...
    private byte[] padTheMessage(byte[] data, int lengthPad)
    {
        ...
        thePad[0] = (byte) 0x80;
        long lengthInBits = origLength * 8 + lengthPad;
